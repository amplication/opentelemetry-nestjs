...
| `docs/` | Markdown guides (e.g., [`docs/migration-5-to-6.md`](docs/migration-5-to-6.md)) plus images referenced by the README. | Keep docs assets (`docs/log.png`, `docs/trace-flow.jpeg`) named descriptively and update links when moving images. |
| `.github/workflows/ci.yml` | CI pipeline running lint, tests (with coverage), and build on push/PR. | All PRs must pass this workflow; mirror its steps locally. |
| `.github/workflows/release.yml` | Semantic-release pipeline for `main`, publishes stable packages with changelog notes. | Runs automatically on `main` only; changing release behavior requires updating docs. |
| `.github/workflows/releaseBeta.yml` | Manual-dispatch semantic-release workflow for beta packages. | Coordinate before triggering; used for pre-release validation. |
| Root configs | `package.json`, `tsconfig*.json`, `.eslintrc.js`, `.prettierrc`, `jest.config.js`. | Scripts (`prebuild`, `build`, `prepublishOnly`, `format`, `lint --fix`, `test`, `test:watch`, `test:cov`, `semantic-release`) define the canonical toolchain. |
...
3. **Types & Style:**
   ...
   - Keep Nest CLI and Jest settings aligned with `tsconfig*.json` and `jest.config.js`.
...
4. **Trace Utilities:** Update `meta-scanner`, `trace-wrapper`, `logger.interface`, and `noop.trace-exporter` in tandem with instrumentation changes, and keep their Jest specs current so wrappers continue to support Promises and Observables.
5. **SDK Helpers:** When adjusting `startNestJsOpenTelemetrySDK` or related helpers, keep defaults backwards-compatible and document new options in `README.md` and `docs/` where relevant.
6. **Docs Consistency & Assets:** Mirror the tone and structure of `README.md` and `docs/migration-5-to-6.md` (headings, callouts, checklists). Reference image assets (e.g., `docs/log.png`, `docs/trace-flow.jpeg`) via relative paths, and keep filenames descriptive if you add or replace media.
7. **Scripts & Tooling:** Use `npm run format`, `npm run lint -- --fix` (if needed), `npm run test`, `npm run test:watch`, `npm run test:cov`, `npm run build`, and `npm run semantic-release` exactly as defined in `package.json`; do not introduce ad-hoc commands without updating this guide.
8. **No Generated Files:** Never commit build outputs. Use `npm run build` only for verification (cleans `dist/` via `prebuild`).
...
const wrapper = new TraceWrapper(traceService, scanner, logger);
wrapper.wrapClass(SomeProvider);
```

- `MetaScanner` caches decorator metadata lookups so instrumentation and `TraceWrapper` do not rescan modules repeatedly.
- `TraceWrapper` supports sync methods, Promises, and Observables; ensure specs cover each path along with error logging via `ILogger` implementations (`logger.interface.ts`).
- Use `NoopTraceExporter` when tests require a SpanExporter without sending telemetry.
...
| Area | Requirements |
| --- | --- |
| **Linting** | `npm run lint` must pass (ESLint + `@typescript-eslint`, Prettier integration). Use `npm run format` to address formatting drift. |
| **Testing** | `npm run test` / `npm run test:cov` using Jest (supports `npm run test:watch` for iteration). Global coverage thresholds: **80% branches**, **90% lines** (`jest.config.js`). |
| **Build** | `npm run build` executes `nest build` after cleaning `dist/` via `prebuild` (`rimraf dist`). Build must succeed with strict TypeScript settings from `tsconfig*.json`. |
...
| **Releases** | `semantic-release` (stable via `release.yml`, beta via `releaseBeta.yml`) expects conventional commits; do not publish manually outside these workflows. |
...
6. Update trace utilities (`meta-scanner`, `trace-wrapper`, `logger.interface`, `noop.trace-exporter`) and their specs if instrumentation relies on new metadata or wrapping behavior.
...
- Keep links relative (e.g., `./docs/log.png`, `./docs/trace-flow.jpeg`).
- Store any new media under `docs/` with descriptive filenames and update references in README/docs accordingly.
...
- [Docs Assets (`docs/log.png`, `docs/trace-flow.jpeg`)](docs) — keep filenames descriptive and update references when replacing graphics.
